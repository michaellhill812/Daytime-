...
for (const vp of [{w:390,h:844,name:'iPhone'},{w:1280,h:900,name:'desktop'}]) {
  ...
  check(`[${vp.name}] the search bar clears the floating buttons`, !overlaps(search, hud),
  ...
  check(`[${vp.name}] the search sits below the button band`, search.y >= hud.y + hud.height,
  ...
  check(`[${vp.name}] no add tile is left in the grid`, (await p.locator('.card--add').count()) === 0);
  ...
  check(`[${vp.name}] nothing scrolls sideways`,
    !(await p.evaluate(()=>document.documentElement.scrollWidth > document.documentElement.clientWidth)));

  await p.screenshot({ path:`${OUT}/wall-${vp.name}.png`, clip:{x:0,y:0,width:vp.w,height:Math.min(vp.h,430)} });
  await p.close();
}
...
await p.locator('.wall__add').click();
await p.waitForTimeout(500);
const opt = await p.locator('#new-doc-domain option').first().evaluate(e=>{
  const s = getComputedStyle(e);
  return { bg: s.backgroundColor, color: s.color };
});
console.log('option style:', JSON.stringify(opt));
check('options have an opaque background', !/rgba\(.*0\)$/.test(opt.bg) && opt.bg !== 'rgba(0, 0, 0, 0)', opt.bg);
check('option text is not the same as its background', opt.bg !== opt.color, `${opt.bg} vs ${opt.color}`);
await p.close();
