...
const wall = sheet
  .locator('.block')
  .filter({ has: p.locator('.block__title', { hasText: 'From the Wall' }) });
...
check(
  'attached ones sort to the front',
...
check(
  'the Wall card now shows a reference count',
  (await card.locator('.card__refs').count()) === 1,
  title,
);
...
const after = p
  ...
  .filter({ has: p.locator('.block__title', { hasText: 'From the Wall' }) });
check(
  'the detach persisted across a reload',
