...
const seeded = await p0.evaluate(() => {
  ...
  s.docs.push({ id:'doc_probe', title:'From Leila', kind:'note', pinned:false,
    updatedAt: at.toISOString(), createdAt: at.toISOString(), createdBy:'leila@example.com' });
...
check('it opens the field guide',
  (await info.getAttribute('href')) === 'https://claude.ai/code/artifact/d645ba04-afed-4048-b451-7d62c864d52d',
...
check('they are packed, not overlapping', bb.x - (ib.x + ib.width) === 10,
  `gap ${bb.x - (ib.x + ib.width)}`);
check('the row clears the account button slot',
  bb.x + bb.width <= 390 - 50, `bell ends ${Math.round(bb.x+bb.width)}`);
...
check('the unread badge sits on the bell, not adrift',
